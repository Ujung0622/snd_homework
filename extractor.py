...
def parse_llm_output(raw_text: str) -> dict:
    ...
    # 마크다운 코드블록 제거 (모델이 실수로 넣었을 경우 대비)
    cleaned = re.sub(r"```(?:json)?", "", raw_text).strip().rstrip("`").strip()
    try:
        return json.loads(cleaned)
    except json.JSONDecodeError as e:
        print(f"[ERROR] LLM 응답을 JSON으로 파싱할 수 없습니다: {e}")
        print(f"[DEBUG] 원문:\n{raw_text[:500]}")
        sys.exit(1)
...
def print_results(data: dict, validation: dict, show_issues: bool = True):
    ...
    if issue_list:
        print(f"\n  ⚠️  검증 경고 항목 {len(issue_list)}개:")
        for v in issue_list:
            print(f"    - [{v['index']+1}] {v['task'][:40]}")
            for iss in v["issues"]:
                print(f"         → {iss}")
    else:
...
def export_json(data: dict, output_path: str):
    ...
    print(f"[INFO] JSON 파일로 저장됨: {output_path}")
...
def main():
    ...
    if not transcript_path.exists():
        print(f"[ERROR] 파일을 찾을 수 없습니다: {transcript_path}")
        sys.exit(1)

    transcript = transcript_path.read_text(encoding="utf-8")
    print(f"[INFO] 회의록 로드 완료: {transcript_path} ({len(transcript)}자)")

    # LLM 호출
    print("[INFO] LLM 분석 중... (수 초 소요)")
    ...
    if not all(checks.values()):
        print("[WARNING] 일부 최소 수량 기준을 충족하지 못했습니다. 위 검증 결과를 확인하세요.")
        sys.exit(2)
