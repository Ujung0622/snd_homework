...
```bash
# 기본 실행 (meeting_transcript.md 파일 사용)
python extractor.py

# 다른 회의록 파일 지정
python extractor.py path/to/my_meeting.md

# JSON으로 결과 저장
python extractor.py --export-json result.json

# 검증 경고 없이 출력
python extractor.py --no-validation-warnings
```
...
| 인증 실패 | 키 확인 안내 후 종료 |
| 네트워크 오류 | 연결 오류 안내 후 종료 |
| JSON 파싱 실패 | 원문 500자 출력 후 종료 |
